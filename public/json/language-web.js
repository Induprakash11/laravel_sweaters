$(document).ready(function () {
    ...
    if($('#language-web').length > 0) {
        ...
        if ( $.fn.DataTable.isDataTable('#language-web') ) {
            $('#language-web').DataTable().destroy();
        }
		$('#language-web').DataTable({
			"bFilter": false, 
				...
				initComplete: (settings, json)=>{
					// Move pagination and length controls, then hide originals
					var $paginate = $('.dataTables_paginate').appendTo('.datatable-paginate');
					var $length = $('.dataTables_length').appendTo('.datatable-length');
					$('#language-web_paginate').hide();
					$('#language-web_length').hide();
				},  
				"data":[
					{
						"id" : 1,
